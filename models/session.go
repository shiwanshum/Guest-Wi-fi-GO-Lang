...
func CreateSession(session *GuestSession) error {
	query := `INSERT INTO guest_sessions (name, mobile, company, purpose, otp) VALUES (?, ?, ?, ?, ?)`
	res, err := DB.Exec(query, session.Name, session.Mobile, session.Company, session.Purpose, session.OTP)
	if err != nil {
		return err
	}
...
func VerifySessionOTP(mobile, otp string, sessionData *GuestSession) (*GuestSession, error) {
	...
	query := `SELECT id, name, mobile, company, purpose, otp, is_verified FROM guest_sessions WHERE mobile = ? AND otp = ? AND is_verified = FALSE ORDER BY id DESC LIMIT 1`
	row := DB.QueryRow(query, mobile, otp)

	var s GuestSession
	err := row.Scan(&s.ID, &s.Name, &s.Mobile, &s.Company, &s.Purpose, &s.OTP, &s.IsVerified)
	if err != nil {
		return nil, err
	}
...
func GetAllSessions() ([]GuestSession, error) {
	query := `SELECT id, name, mobile, company, purpose, is_verified, mac_address, ip_address, device, os, browser, login_time, logout_time, data_download, data_upload FROM guest_sessions ORDER BY id DESC`
	...
	for rows.Next() {
		var s GuestSession
		err := rows.Scan(
			&s.ID, &s.Name, &s.Mobile, &s.Company, &s.Purpose, &s.IsVerified,
